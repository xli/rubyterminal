...
to setup your Rails project environment as you need, but it maybe not what you want to
cache in RubyTerminal, because there maybe lot of models has been loaded and cache those
classes may cause you restart RubyTerminal again and again.
So, I need provide a new config/const_environment.rb to describe everything my project need
and no need to reload while I am running tests in development usually. And then require it
at first line in config/environment.rb.
Also, I added the following code as first line code of config/const_environment.rb

  require 'ert'

Then, start RubyTerminal in my project root directory with my config/const_environment.rb
when I am developing.

  rt config/const_environment.rb
