...
Setup my project by adding one line code into first line code of test_helper.rb
...
So, any change in side your config/environment.rb would not be reloaded in RubyTerminal. In this
case, you should restart RubyTerminal to launch new environment.

And for the way how 'require' works in ruby, I highly recommend you require all file in same file path
format. For example, if you require your 'config/environment.rb' as above, and use the following code
to require it again in your ruby execution script:

  require 'config/environment'

You would cause the 'config/environment.rb' to be reloaded inside your ruby execution script, which
may cause you wait for long time launching the process.
